...
from modeling_roberta_context import RobertaForContext # TODO
...
PREVQ1_TOKEN = CLS_TOKEN
PREVQ2_TOKEN = CLS_TOKEN
PREVQ3_TOKEN = CLS_TOKEN
...
def load_checkpoint(model, state_dict):
    ...
    old_keys = []
    new_keys = []
#     print("\n ==============all state_dict in reloaded model: ", state_dict.keys())
    ...
    if len(missing_keys) > 0:
        print("Weights of {} not initialized from reloaded pretrained model: {}".format(
            model.__class__.__name__, missing_keys))
    if len(unexpected_keys) > 0:
        print("Weights reloaded from pretrained model not used in {}: {}".format(
...
def load_roberta(model_path):
    bert_config_file = os.path.join(model_path, f'config_3.json')
    ...
    model_bert = RobertaForContext(bert_config)
    state_dict = torch.load(init_checkpoint, map_location='cpu')
    model_bert = load_checkpoint(model_bert, state_dict)
    print("\n===============Loaded pre-trained model parameters.")
...
class BERTDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        q_label = line.split("|||")
        assert len(q_label) == 2 or len(q_label) == 3
        
        if len(q_label) == 2:
            t1, label_str = q_label[0].strip(), q_label[1].strip()
            context_labels = []
        else:
            t1, label_str, context_label_str = q_label[0].strip(), q_label[1].strip(), q_label[2].strip()
            context_labels = [int(i) for i in context_label_str.split(" ")]
            
        raw_label = [int(i) for i in label_str.split(" ")]
        
        t1 = t1.replace("<unk>", CLS_TOKEN)
        # tokenize
        tokens_a = self.tokenizer.tokenize(t1)

        # combine to one sample
        cur_example = InputExample(guid=cur_id, tokens_a=tokens_a, raw_label=raw_label, context_labels=context_labels)
        ...
        cur_tensors = (torch.tensor(cur_features.input_ids),
                       ...
                       torch.tensor(cur_features.context_label_ids),
...
class InputExample(object):
    ...
    def __init__(self, guid, tokens_a, raw_label=None, context_labels=None):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, token_type_ids, col_label_ids, context_label_ids, lm_label_ids, q_tab_inds):
...
def random_word(tokens, tokenizer, col_start_index):
    """
    ...
    for i, token in enumerate(tokens):
        ...
        if token != CLS_TOKEN and token != SEP_TOKEN and token != SEP_TOKEN and prob < 0.15: #and i < col_start_index
            prob /= 0.15
...
def convert_example_to_features(example, max_seq_length, tokenizer, mlm_loss):
    ...
    raw_label = raw_label[:len(col_ids)]
    ...
    # for context labels
    context_labels = example.context_labels
    prevq_ids = [i for i, x in enumerate(tokens_a) if x == CLS_TOKEN]
    context_label_ids = [-1] * len(tokens_a)
    if len(context_labels) != 0:
        assert len(context_labels) == len(prevq_ids)
        for qid, clb in zip(prevq_ids, context_labels):
            context_label_ids[qid] = clb
    ...
    for i, x in enumerate(tokens_a):
        if (i != 0 and x == CLS_TOKEN) or (x == SEP_TOKEN and sep_count == 0):
            prevq_count += 1
        
        if x == SEP_TOKEN:
            sep_count += 1
            
        token_type_ids[i] = prevq_count % 2
    ...
    #mlm loss only for natural questions
    lm_label_ids = [-1]*len(tokens_a)
    if SEP_TOKEN in tokens_a: # TODO, some lines don't have columns
        col_start_index = tokens_a.index(SEP_TOKEN)
    else:
        col_start_index = len(tokens_a) - 1
    if all(i == -1 for i in col_label_ids):
        tokens_b, lm_label_ids = random_word(tokens_a, tokenizer, col_start_index)
        if mlm_loss:
            tokens_a = tokens_b
    ...
    assert len(context_label_ids) == max_seq_length

    if example.guid < 5:
        print("If using MLM loss: ", mlm_loss)
        print("*** Example ***")
        print("guid: %s" % (example.guid))
        print("tokens_a: %s" % " ".join(tokens_a))
        print("input_ids: %s" % " ".join([str(x) for x in input_ids]))
        print("input_mask: %s" % " ".join([str(x) for x in input_mask]))
        print("token_type_ids: %s" % " ".join([str(x) for x in token_type_ids]))
        print("col_label_ids: %s" % " ".join([str(x) for x in col_label_ids]))
        print("context_label_ids: %s" % " ".join([str(x) for x in context_label_ids]))
...
def main():
    ...
    parser.add_argument("--diff_loss",
                        action='store_true',
                        help="Whether to add sql diff loss.")
    parser.add_argument("--token_type",
                        action='store_true',
                        help="Whether to add token type ids in roberta.")
...
#         model, tokenizer = load_roberta(args.reload_model)
#     else:
    tokenizer = RobertaTokenizer.from_pretrained(args.bert_model, do_lower_case=args.do_lower_case)
    model = RobertaForContext.from_pretrained(args.bert_model)
...
#     wandb.watch(model)
    ...
    if args.do_train:
        ...
        print("  diff loss:", args.diff_loss)
        print("  mlm loss:", args.mlm_loss)
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                model.train()
                input_ids, input_mask, token_type_ids, col_label_ids, context_label_ids, lm_label_ids, q_tab_inds = batch
                input_ids, input_mask, token_type_ids, col_label_ids, context_label_ids, lm_label_ids, q_tab_inds = to_device(device, input_ids, input_mask, token_type_ids, col_label_ids, context_label_ids, lm_label_ids, q_tab_inds)
                ...
                if args.mlm_loss and args.diff_loss:
                    loss = model(input_ids=input_ids,
                                attention_mask=input_mask,
                                token_type_ids=token_type_ids,
                                masked_lm_labels=lm_label_ids,
                                masked_col_labels=col_label_ids,
                                masked_context_labels=context_label_ids,
                                q_tab_inds=q_tab_inds)
                elif args.diff_loss:
                    loss = model(input_ids=input_ids,
                                attention_mask=input_mask,
...
#             wandb.log({'epoch_training_loss': epoch_loss})
            ...
            if args.do_eval and args.local_rank in [-1, 0]:
                ...
                for step, batch in enumerate(train_eval_dataloader):
                    model.eval()
                    input_ids, input_mask, token_type_ids, col_label_ids, context_label_ids, _, q_tab_inds = batch
                    input_ids, input_mask, token_type_ids, col_label_ids, context_label_ids, q_tab_inds = to_device(device, input_ids, input_mask, token_type_ids, col_label_ids, context_label_ids, q_tab_inds)
