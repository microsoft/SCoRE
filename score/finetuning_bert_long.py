...
from modeling_bert_context_long import BertForContext # TODO
...
class BERTDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        t1 = t1.replace("<unk>", CLS_TOKEN).replace("<s>", CLS_TOKEN).replace("</s>", SEP_TOKEN)
        # tokenize
    ...
    for i, token in enumerate(tokens):
        ...
        if token != CLS_TOKEN and token != SEP_TOKEN and token != SEP_TOKEN and prob < 0.15 and i < col_start_index:
            ...
            # 10% randomly change token to random token
                ...
                tokens[i] = random.choice(list(tokenizer.vocab))
            ...
            try:
                ...
                output_label.append(tokenizer.convert_tokens_to_ids(UNK_TOKEN))
                logger.warning(
...
def convert_example_to_features(example, max_seq_length, tokenizer, mlm_loss):
    ...
    # CHECK: </s> appears
    col_ids = [i for i, x in enumerate(tokens_a) if x == SEP_TOKEN][2:-1]
    ...
    if len(col_ids) > 0: # TODO, some lines don't have columns
        col_start_index = col_ids[0]
    else:
        col_start_index = len(tokens_a) - 1
    if all(i == -1 for i in col_label_ids) or len(raw_label) == 0 or len(col_ids) == 0:
    ...
    while len(input_ids) < max_seq_length:
        input_ids.append(PAD_ID) # roberta pad index is 1
        input_mask.append(0)
...
def main():
    ...
    parser.add_argument("--col_loss",
                        action='store_true',
                        help="Whether to add col sql loss.")
...
#     else:
    ...
    model = BertForContext.from_pretrained(args.bert_model)
    ...
    if args.do_train:
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if args.mlm_loss and args.col_loss:
